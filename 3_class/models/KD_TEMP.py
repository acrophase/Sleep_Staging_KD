...
class SD_CL_model(LightningModule):
    def __init__(
        self,
        filters=None,
        in_channels=None,
    ...
    def training_step(self, batch_train, batch_idx):
        ...
        train_Temp_loss_ecg, train_feature_loss, train_loss_eeg, train_loss_ecg, pred_train_eeg, pred_train_ecg, y_train = self.compute_loss(pred_train_eeg, feature_list_eeg, pred_train_ecg, feature_list_ecg, y_train, class_weights)
        ...
        ## Logging losses
        self.log('train_loss', train_Temp_loss_ecg,             on_step=True,  on_epoch=True, prog_bar=False, logger=True, sync_dist=True)
        ...
        self.log('train_loss_ecg', train_loss_ecg,              on_step=True,  on_epoch=True, prog_bar=False, logger=True, sync_dist=True)
       
        return {
            'loss':train_Temp_loss_ecg,
        }
    ...
    def validation_step(self, batch_eval, batch_idx):
        ...
        val_Temp_loss_ecg, val_feature_loss, val_loss_eeg, val_loss_ecg, pred_val_eeg, pred_val_ecg, y_val = self.compute_loss(pred_val_eeg, feature_list_eeg,  pred_val_ecg, feature_list_ecg, y_val,class_weights)
        
        self.pred_val_acc = torch.cat([self.pred_val_acc,pred_val_ecg], dim = 0)
        self.y_val_acc = torch.cat([self.y_val_acc,y_val], dim = 0)

        ## Logging metrics
        self.log('val_loss', val_Temp_loss_ecg,             on_step=True,  on_epoch=True, prog_bar=False, logger=True, sync_dist=True)
        ...
        self.log('val_loss_ecg', val_loss_ecg,              on_step=True,  on_epoch=True, prog_bar=False, logger=True, sync_dist=True)
        
        return {
            'val_loss': val_Temp_loss_ecg,
        }
    ...
    def test_step(self, batch_test, batch_idx):
        ...
        test_Temp_loss_ecg, test_feature_loss, test_loss_eeg, test_loss_ecg, pred_test_eeg, pred_test_ecg, y_test = self.compute_loss(pred_test_eeg, feature_list_eeg,  pred_test_ecg, feature_list_ecg, y_test, class_weights)
        ...
        ## Logging metrics
        self.log('test_loss', test_Temp_loss_ecg,               on_step=True, on_epoch=True, prog_bar=False, logger=True, sync_dist=True)
        ...
        self.log('test_loss_ecg', test_loss_ecg,                on_step=True,  on_epoch=True, prog_bar=True, logger=True, sync_dist=True)
        
        
        return {
            'test_loss': test_Temp_loss_ecg,
        }
    ...
    def compute_loss(self, y_pred_eeg, y_feature_list_eeg, y_pred_ecg, y_feature_list_ecg, y_true, class_weights):
        ...
        ## TEMPERATURE LOSS with CROSS_ENTROPY
        def loss_fn_kd(outputs, labels, teacher_outputs, alpha, Temp):
            ...
            KD_loss = nn.KLDivLoss()(F.log_softmax(outputs/T, dim=1), F.softmax(teacher_outputs/T, dim=1)) * (alpha * T * T) + \
                    self.wce_loss(outputs, labels) * (1. - alpha)
        ...
        Temp_loss_ecg = loss_fn_kd(y_pred_ecg.squeeze(1), torch.argmax(y_true, dim = 2).squeeze(1), y_pred_eeg.squeeze(1), alpha, Temp)
        
        return Temp_loss_ecg, feat_loss, loss_eeg, loss_ecg, y_pred_eeg, y_pred_ecg, y_true
    ...
    def add_model_specific_args(parent_parser):
        ...
        architecture_group.add_argument("--eeg_baseline_path", default= None, help= 'Enter path to saved EEG baseline file')
...
if __name__ == "__main__":
    ...
    parser = SD_CL_model.add_model_specific_args(parser)
    args = parser.parse_args()

    utime =SD_CL_model(vars(args))