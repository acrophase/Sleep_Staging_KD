...
class ECG_BASE_Model(LightningModule):
    # def __init__(
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.train_weights = weights_list
        self.val_weights = weights_list
        self.test_weights = weights_list
        self.epoch_length = epoch_length
    ...
    def training_step(self, batch_train, batch_idx):
        ...
        ecg_train = ecg_train.squeeze(0).unsqueeze(1)
        y_train = y_train.squeeze(0).squeeze(0)
        pred_train = self.classify_segments(ecg_train.float(), resolution=self.hparams.epoch_length)
    ...
    def validation_step(self, batch_eval, batch_idx):
        ...
        ecg_val = ecg_val.squeeze(0).unsqueeze(1)
        y_val = y_val.squeeze(0).squeeze(0)
        pred_val = self.classify_segments(ecg_val.float(), resolution=self.hparams.epoch_length)
    ...
    def test_step(self, batch_test, batch_idx):
        ...
        ecg_test = ecg_test.squeeze(0).unsqueeze(1)
        y_test = y_test.squeeze(0).squeeze(0)
        pred_test = self.classify_segments(ecg_test.float(), resolution=self.hparams.epoch_length)
        ...
        y_1s = self.classify_segments(ecg_test.float(), resolution=1)
...
if __name__ == "__main__":
    ...
    parser = ECG_BASE_Model.add_model_specific_args(parser)
    args = parser.parse_args()

    utime = ECG_BASE_Model(vars(args))